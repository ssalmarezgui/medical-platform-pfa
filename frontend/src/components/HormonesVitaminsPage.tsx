...
import { useHormoneHistory, useSaveHormones, useDeleteHormones } from '../features/hormones-vitamins/hooks/useHormones';
import { usePatients } from '../features/patients/hooks/usePatients';
import { useQueryClient } from '@tanstack/react-query';
import { hormoneService } from '../features/hormones-vitamins/api/hormoneService';
import { 
...
export const HormonesVitaminsPage = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[500px] text-center p-8 bg-white border border-red-100 rounded-[30px] shadow-sm max-w-lg mx-auto mt-12 text-xs">
        ...
        <p className="text-[#6588BB] text-sm leading-relaxed mb-6">
          La saisie, la validation et le dosage des hormones et vitamines (PTH, TSH, Vitamine D...) relèvent de la responsabilité légale exclusive du biologiste ou des agents de laboratoire.
        </p>
      </div>
    );
  }
  ...
  const { data: donorHormone, isLoading: loadingDonorHormones } = useDonorHormoneHistory(
    isDonorMode ? (selectedPatientId || undefined) : undefined
  );

  const hormone = isDonorMode ? donorHormone : patientHormone;
  const loadingHormones = isDonorMode ? loadingDonorHormones : loadingPatientHormones;
  ...
  useEffect(() => {
    if (hormone) {
      ...
      setAnalysesSaisies(hormone.analyses || []);
    } else {
  ...
  const handleAddAnalysisRow = () => {
    ...
    const nouvelleLigne = {
      ...
      valeurAna,
      typeAnalyse: 'HORMONES_VITAMINES'
    };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isDonorMode) {
        await axios.post(`http://localhost:8081/api/hormones-vitamines/donneur/${selectedPatientId}`, payload);
      } else {
        await saveMutation.mutateAsync({ patientId: patientIdString, data: payload });
      }

      setToastType('success');
      setToastMessage("La fiche d'hormones & vitamines a été enregistrée avec succès !");
      setToastOpen(true);

      queryClient.invalidateQueries({ queryKey: ['hormoneHistory'] });
      queryClient.invalidateQueries({ queryKey: ['donorHormoneHistory'] });
    } catch {
      setToastType('error');
  ...
  const handleDelete = async () => {
    if (!hormone?.identifiantHV) return;
    try {
      if (isDonorMode) {
        await axios.delete(`http://localhost:8081/api/hormones-vitamines/${hormone.identifiantHV}`);
      } else {
        await deleteMutation.mutateAsync(hormone.identifiantHV);
      }
  ...
  const handleExportCSV = async (mode: 'global' | 'personal') => {
    ...
    if (mode === 'personal' && selectedPatientId) {
      ...
      filename = `export_hormonologie_${isDonorMode ? 'donneur' : 'patient'}_${selectedPatientId}_${new Date().toISOString().split('T')[0]}.csv`;
    } else {
      try {
        const globalData = await hormoneService.getAll();
        datasetToExport = globalData || [];
        filename = `export_global_hormonologie_cohorte_${new Date().toISOString().split('T')[0]}.csv`;
      } catch {
    ...
    if (datasetToExport.length === 0) {
      alert("Aucune analyse d'hormonologie à exporter.");
      return;
    }
    ...
    if (mode === 'personal') {
      ...
      const csvRows = datasetToExport.map(row => [
        ...
        row.typeAnalyse || 'HORMONES_VITAMINES'
      ].join(";"));
      csvContent = "\ufeff" + [headers.join(";"), ...csvRows].join("\n");
    } else {
      const headers = ["patientId", "typeHV", "dateAna", "resultatAna", "valeurAna", "typeAnalyse"];
      const csvRows: string[] = [];
      datasetToExport.forEach(hv => {
        const patientId = hv.patientId || hv.patient?.identifiantP || "";
        const lib = hv.typeHV || "Hormones et Vitamines";
        
        if (hv.analyses && hv.analyses.length > 0) {
          hv.analyses.forEach((a: any) => {
            csvRows.push([
              patientId, `"${lib.replace(/"/g, '""')}"`,
              a.dateAna || "", `"${a.resultatAna.replace(/"/g, '""')}"`, `"${a.valeurAna.replace(/"/g, '""')}"`,
              a.typeAnalyse || 'HORMONES_VITAMINES'
            ].join(";"));
          });
        } else {
          csvRows.push([patientId, `"${lib.replace(/"/g, '""')}"`, "", "", "", "HORMONES_VITAMINES"].join(";"));
        }
      });
      csvContent = "\ufeff" + [headers.join(";"), ...csvRows].join("\n");
    }
  ...
  const handleDownloadTemplate = (mode: 'global' | 'personal') => {
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = mode === 'global' ? "modele_import_hormones_global.csv" : "modele_import_hormones_personnel.csv";
    link.click();
    URL.revokeObjectURL(url);
  };
    ...
    if (importMode === 'personal') {
      ...
      previewData.forEach(row => {
        nouvellesAnalyses.push({
          ...
          resultatAna: row.resultatAna || listExamensHormones[0],
      ...
      previewData.forEach(row => {
        ...
        if (!mappedGroups[pid]) {
          mappedGroups[pid] = {
            typeHV: row.typeHV || "Hormones et Vitamines",
            analyses: []
          };
        }
      ...
      for (const [pid, data] of Object.entries(mappedGroups)) {
        try {
          await hormoneService.save(pid, data);
          successCount++;
        } catch (err) {
          console.error("Erreur import global :", err);
        }
      }
      setToastMessage(`${successCount} fiche(s) d'hormonologie de cohorte importée(s) !`);
    }
        ...
        <div className="bg-white border border-[#A7C0E4]/30 rounded-[30px] p-8 shadow-sm">
          <div className="mb-6">
            <h3 className="text-base font-bold text-slate-800">
              {isDonorMode ? "Registre d'Hormonologie de votre Cohorte Donneurs" : "Registre d'Hormonologie de votre Cohorte Patients"}
            </h3>
            <p className="text-[#6588BB] text-xs mt-0.5 font-semibold">
              {isDonorMode ? "Suivez et consignez le dosage des hormones et des vitamines de vos donneurs." : "Suivez et consignez le dosage des hormones et des vitamines de vos patients."}
            </p>
          </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b">
                <div>
                  <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                    ...
                    Registre d'Hormonologie ({isDonorMode ? `${selectedPatient?.prenomD} ${selectedPatient?.nomD}` : `${selectedPatient?.prenomP} ${selectedPatient?.nomP}`})
                  </h3>
                </div>
