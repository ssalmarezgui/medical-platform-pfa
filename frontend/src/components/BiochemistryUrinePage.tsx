...
import { useUrineHistory, useSaveUrine, useDeleteUrine } from '../features/biochemistry-urine/hooks/useUrine';
import { usePatients } from '../features/patients/hooks/usePatients';
import { useQueryClient } from '@tanstack/react-query';
import { urineService } from '../features/biochemistry-urine/api/urineService';
import { 
  IconTestPipe, IconSearch, IconPlus, IconTrash, IconUserCheck, IconAlertCircle, IconCalendar, IconReportMedical, IconEdit,
...
export const BiochemistryUrinePage = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[500px] text-center p-8 bg-white border border-red-100 rounded-[30px] shadow-sm max-w-lg mx-auto mt-12 text-xs">
        ...
        <p className="text-[#6588BB] text-sm leading-relaxed mb-6">
          La saisie et la validation des constantes de biochimie urinaire (protéinurie des 24h, créatininurie) relèvent de la responsabilité légale exclusive du biologiste ou des agents de laboratoire.
        </p>
      </div>
    );
  }
  ...
  const { data: patientUrine, isLoading: loadingPatientUrine } = useUrineHistory(
    !isDonorMode ? (patientIdString || undefined) : undefined
  );

  const { data: donorUrine, isLoading: loadingDonorUrine } = useDonorUrineHistory(
    isDonorMode ? (selectedPatientId || undefined) : undefined
  );

  const urine = isDonorMode ? donorUrine : patientUrine;
  const loadingUrine = isDonorMode ? loadingDonorUrine : loadingPatientUrine;

  const saveMutation = useSaveUrine();
  const deleteMutation = useDeleteUrine(patientIdString);

  const [localSearchTerm, setLocalSearchTerm] = useState('');

  const listExamensUrines = [
    "Na/K urines",
    "Protéinurie",
    "Autres (urines)",
    "Autre Fluide"
  ...
  const [libelleBUF, setLibelleBUF] = useState("Biochimie Urinaire et Fluides");
  const [descriptionBUF, setDescriptionBUF] = useState('');
  
  const [selectedExamen, setSelectedExamen] = useState(listExamensUrines[0]);
  ...
  useEffect(() => {
    if (urine) {
      setLibelleBUF(urine.libelleBUF);
      setDescriptionBUF(urine.descriptionBUF || '');
      setAnalysesSaisies(urine.analyses || []);
    } else {
      setLibelleBUF("Biochimie Urinaire et Fluides");
      setDescriptionBUF('');
      setAnalysesSaisies([]);
    }
  }, [urine]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isDonorMode) {
        await urineService.saveForDonor(selectedPatientId, payload);
      } else {
        await saveMutation.mutateAsync({ patientId: patientIdString, data: payload });
      }

      setToastType('success');
      setToastMessage("La fiche de biochimie urinaire a été enregistrée avec succès !");
      setToastOpen(true);
  ...
  const handleDelete = async () => {
    if (!urine?.identifiantBUF) return;
    try {
      await urineService.delete(urine.identifiantBUF);
      setConfirmOpen(false);
      setToastType('success');
      setToastMessage("Fiche de biochimie urinaire supprimée.");
      setToastOpen(true);
      ...
      queryClient.removeQueries({ queryKey: ['urineHistory'] });
      queryClient.removeQueries({ queryKey: ['donorUrineHistory'] });
    } catch {
      setConfirmOpen(false);
      setToastType('error');
  ...
  const handleExportCSV = async (mode: 'global' | 'personal') => {
    ...
    if (mode === 'personal' && selectedPatientId) {
      ...
      filename = `export_biochimie_urinaire_${isDonorMode ? 'donneur' : 'patient'}_${selectedPatientId}_${new Date().toISOString().split('T')[0]}.csv`;
    } else {
      try {
        const globalData = await urineService.getAll();
        datasetToExport = globalData || [];
        filename = `export_global_biochimie_urinaire_cohorte_${new Date().toISOString().split('T')[0]}.csv`;
      } catch {
    ...
    if (datasetToExport.length === 0) {
      alert("Aucune donnée urinaire à exporter.");
      return;
    }
    ...
    if (mode === 'personal') {
      ...
      const csvRows = datasetToExport.map(row => [
        ...
        row.typeAnalyse || 'BIOCHIMIE_URINE'
      ].join(";"));
      csvContent = "\ufeff" + [headers.join(";"), ...csvRows].join("\n");
    } else {
      const headers = ["patientId", "libelleBUF", "descriptionBUF", "dateAna", "resultatAna", "valeurAna", "typeAnalyse"];
      const csvRows: string[] = [];
      datasetToExport.forEach(bu => {
        const patientId = bu.patientId || bu.patient?.identifiantP || "";
        const lib = bu.libelleBUF || "Biochimie Urinaire et Fluides";
        const desc = bu.descriptionBUF || "";
        
        if (bu.analyses && bu.analyses.length > 0) {
          bu.analyses.forEach((a: any) => {
            csvRows.push([
              patientId, `"${lib.replace(/"/g, '""')}"`, `"${desc.replace(/"/g, '""')}"`,
              a.dateAna || "", `"${a.resultatAna.replace(/"/g, '""')}"`, `"${a.valeurAna.replace(/"/g, '""')}"`,
              a.typeAnalyse || 'BIOCHIMIE_URINE'
            ].join(";"));
          });
        } else {
          csvRows.push([patientId, `"${lib.replace(/"/g, '""')}"`, `"${desc.replace(/"/g, '""')}"`, "", "", "", "BIOCHIMIE_URINE"].join(";"));
        }
      });
      csvContent = "\ufeff" + [headers.join(";"), ...csvRows].join("\n");
    }
  ...
  const handleDownloadTemplate = (mode: 'global' | 'personal') => {
    const headers = mode === 'global' 
      ? ["patientId", "libelleBUF", "descriptionBUF", "dateAna", "resultatAna", "valeurAna"]
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = mode === 'global' ? "modele_import_urine_global.csv" : "modele_import_urine_personnel.csv";
    link.click();
    URL.revokeObjectURL(url);
  };
  ...
  const handleImportSubmit = async () => {
    ...
    if (importMode === 'personal') {
      ...
      previewData.forEach(row => {
        nouvellesAnalyses.push({
          ...
          resultatAna: row.resultatAna || listExamensUrines[0],
      ...
      previewData.forEach(row => {
        ...
        if (!mappedGroups[pid]) {
          mappedGroups[pid] = {
            libelleBUF: row.libelleBUF || "Biochimie Urinaire et Fluides",
            descriptionBUF: row.descriptionBUF || '',
            analyses: []
          };
        }
      ...
      for (const [pid, data] of Object.entries(mappedGroups)) {
        try {
          await urineService.save(pid, data);
          successCount++;
        } catch (err) {
          console.error("Erreur import global :", err);
        }
      }
      setToastMessage(`${successCount} fiche(s) de biochimie urinaire synchronisée(s) !`);
    }
    ...
    queryClient.invalidateQueries({ queryKey: ['urineHistory'] });
    queryClient.invalidateQueries({ queryKey: ['donorUrineHistory'] });
  };
  ...
  return (
    <div className="max-w-[1200px] mx-auto py-8 text-xs">
      ...
      <div className="bg-white border border-[#A7C0E4]/30 rounded-[30px] p-8 shadow-sm mb-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-[#2B5296] flex items-center gap-2">
            <IconTestPipe size={24} className="text-[#2B5296] animate-pulse" />
            Fiche de Biochimie des Urines & Autres Fluides
          </h2>
        ...
        <div className="bg-white border border-[#A7C0E4]/30 rounded-[30px] p-8 shadow-sm">
          <div className="mb-6">
            <h3 className="text-base font-bold text-slate-800">
              {isDonorMode ? "Registre de Biochimie Urinaire de la Cohorte Donneurs" : "Registre de Biochimie Urinaire de la Cohorte Patients"}
            </h3>
            <p className="text-[#6588BB] text-xs mt-0.5 font-semibold">
              {isDonorMode ? "Consultez et complétez les analyses biochimiques urinaires de vos donneurs." : "Consultez et complétez les analyses biochimiques urinaires de vos patients."}
            </p>
          </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b">
                <div>
                  <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                    <IconTestPipe size={22} className="text-[#2B5296]" />
                    Registre de Biochimie Urinaire ({isDonorMode ? `${selectedPatient?.prenomD} ${selectedPatient?.nomD}` : `${selectedPatient?.prenomP} ${selectedPatient?.nomP}`})
                  </h3>
                </div>
          ...
          <div className="bg-white rounded-[24px] p-8 w-full max-w-lg shadow-2xl space-y-6">
            <div className="flex justify-between items-center border-b pb-4">
              <h2 className="text-lg font-bold text-[#2B5296]">
                {importMode === 'global' ? "Importation de Cohorte Urinaire (CSV)" : `Importer les analyses de ${isDonorMode ? selectedPatient?.prenomD : selectedPatient?.prenomP}`}
              </h2>
      ...
      <DeleteConfirmModal isOpen={confirmOpen} onClose={() => setConfirmOpen(false)} onConfirm={handleDelete} title="Supprimer la fiche urinaire ?" message="Cette action effacera définitivement l'intégralité du bilan de biochimie urinaire ainsi que toutes les lignes de résultats associées." />
