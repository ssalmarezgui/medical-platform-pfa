...
import { useImagingHistory, useCreateImaging, useDeleteImaging } from '../features/imaging/hooks/useImaging';
import { usePatients } from '../features/patients/hooks/usePatients';
import { useQueryClient } from '@tanstack/react-query';
import { imagingService } from '../features/imaging/api/imagingService';
import { 
  IconPhoto, IconSearch, IconPlus, IconTrash, IconUserCheck, IconAlertCircle, IconCalendar, IconReportMedical, IconEdit, IconNotes,
...
export const ImagingHistoryPage = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[500px] text-center p-8 bg-white border border-red-100 rounded-[30px] shadow-sm max-w-lg mx-auto mt-12 text-xs">
        ...
        <p className="text-[#6588BB] text-sm leading-relaxed mb-6">
          La saisie, la mise à jour et l'enregistrement des rapports d'imagerie et examens radiologiques sont réservés exclusivement aux techniciens de laboratoire et agents de radiologie qualifiés.
        </p>
      </div>
    );
  }
  ...
  const { data: donorImaging, isLoading: loadingDonorImaging } = useDonorImagingHistory(
    isDonorMode ? (selectedPatientId || undefined) : undefined
  );

  const imagings = isDonorMode ? donorImaging : patientImaging;
  const loadingImaging = isDonorMode ? loadingDonorImaging : loadingPatientImaging;

  const createMutation = useCreateImaging();
  const deleteMutation = useDeleteImaging(patientIdString);

  const [localSearchTerm, setLocalSearchTerm] = useState('');

  const listExamensPapier = [
    "Rx Thorax",
    ...
    "Coronarographie",
    "Echo-Doppler des axes artériels et veineux",
  ...
  const filteredLocalImagings = imagings?.filter(im => {
    ...
    const resStr = im.resultatIm?.toLowerCase() || '';
    const query = localSearchTerm.toLowerCase();

    return nameStr.includes(query) || dateStr.includes(query) || resStr.includes(query);
  }) || [];
  ...
  const resetForm = () => {
    ...
    setExamenIm(listExamensPapier[0]);
    setDateIm('');
    setResultatIm('');
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedImaging) {
        await imagingService.update(selectedImaging.identifiantIm, payload);
        setToastMessage("Examen d'imagerie mis à jour !");
      } else {
        if (isDonorMode) {
          await axios.post(`http://localhost:8081/api/imageries/donneur/${selectedPatientId}`, payload);
        } else {
          await createMutation.mutateAsync({ patientId: patientIdString, data: payload });
        }
        setToastMessage("Nouvel examen d'imagerie enregistré !");
      }

      setToastType('success');
      setToastOpen(true);
      
      queryClient.invalidateQueries({ queryKey: ['imagingHistory'] });
      queryClient.invalidateQueries({ queryKey: ['donorImagingHistory'] });
  ...
  const triggerEdit = (im: any) => {
    setSelectedImaging(im);
    setExamenIm(im.examenIm);
    setDateIm(im.dateIm);
    setResultatIm(im.resultatIm);
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setToastType('success');
      setToastMessage("Examen d'imagerie supprimé.");
      setToastOpen(true);
      
      queryClient.removeQueries({ queryKey: ['imagingHistory'] });
      queryClient.removeQueries({ queryKey: ['donorImagingHistory'] });
    } catch {
      setConfirmOpen(false);
      setToastType('error');
  ...
  const handleExportCSV = async (mode: 'global' | 'personal') => {
    ...
    if (mode === 'personal' && selectedPatientId) {
      datasetToExport = imagings || [];
      filename = `export_imageries_${isDonorMode ? 'donneur' : 'patient'}_${selectedPatientId}_${new Date().toISOString().split('T')[0]}.csv`;
    } else {
      try {
        const globalData = await imagingService.getByPatientId('');
        datasetToExport = globalData || [];
        filename = `export_global_imageries_cohorte_${new Date().toISOString().split('T')[0]}.csv`;
      } catch {
    ...
    if (datasetToExport.length === 0) {
      alert("Aucune imagerie à exporter.");
      return;
    }

    const headers = ["identifiantIm", "patientId", "examenIm", "dateIm", "resultatIm"];
    const csvRows = datasetToExport.map(im => [
      im.identifiantIm || "",
      im.patientId || im.patient?.identifiantP || "",
      `"${String(im.examenIm || '').replace(/"/g, '""')}"`,
      im.dateIm || "",
      `"${String(im.resultatIm || '').replace(/"/g, '""')}"`
    ].join(";"));
  ...
  const handleDownloadTemplate = (mode: 'global' | 'personal') => {
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = mode === 'global' ? "modele_import_imageries_global.csv" : "modele_import_imageries_personnel.csv";
    link.click();
    URL.revokeObjectURL(url);
  };
  ...
  const handleImportSubmit = async () => {
    ...
    for (const row of previewData) {
      try {
        const payload = {
          examenIm: row.examenIm,
          dateIm: row.dateIm,
          resultatIm: row.resultatIm || 'Aucun résultat spécifié'
        };

        const targetPatientId = importMode === 'global' ? row.patientId : patientIdString;
        
        if (targetPatientId) {
          await imagingService.create(targetPatientId, payload);
          successCount++;
        }
      } catch (err) {
    ...
    setToastMessage(`${successCount} examen(s) d'imagerie importé(s) !`);
    setToastType('success');
    setToastOpen(true);
        ...
        <div className="bg-white border border-[#A7C0E4]/30 rounded-[30px] p-8 shadow-sm">
          <div className="mb-6">
            <h3 className="text-base font-bold text-slate-800">
              {isDonorMode ? "Registre Historique d'Imagerie de la Cohorte Donneurs" : "Registre Historique d'Imagerie de la Cohorte Patients"}
            </h3>
            <p className="text-[#6588BB] text-xs mt-0.5 font-semibold font-sans">Visualisez et importez les dossiers d'imagerie et d'examens médicaux de vos candidats.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center text-slate-400">
              ...
              <p className="font-semibold">Aucun dossier rattaché à votre profil.</p>
            </div>
          )}
        </div>
      ) : (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in duration-300">
          ...
          <div className={`${isReadOnly ? 'lg:col-span-3' : 'lg:col-span-2'} bg-white border border-[#A7C0E4]/30 rounded-[30px] p-8 shadow-sm min-h-[500px] flex flex-col justify-between`}>
            <div>
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b">
                <div>
                  <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                    <IconPhoto size={22} className="text-[#2B5296]" />
                    Dossier d'Imageries & Explorations ({isDonorMode ? `${selectedPatient?.prenomD} ${selectedPatient?.nomD}` : `${selectedPatient?.prenomP} ${selectedPatient?.nomP}`})
                  </h3>
                </div>
              ...
              {loadingImaging ? (
              ...
              ) : filteredLocalImagings && filteredLocalImagings.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {filteredLocalImagings.map((im) => (
                    <div key={im.identifiantIm} className="border border-slate-100 bg-[#F8FAFC]/50 p-5 rounded-2xl flex flex-col justify-between">
                      <div>
                        ...
                        <div className="space-y-1.5 text-xs text-slate-600 mt-4 border-t border-slate-100 pt-3">
                          <p className="flex items-center gap-1.5"><IconCalendar size={14} /> Effectué le : <strong>{im.dateIm}</strong></p>
                          <p className="flex items-start gap-1.5 italic text-slate-500 mt-2"><IconNotes size={14} className="shrink-0 mt-0.5" /> "{im.resultatIm}"</p>
                        </div>
                      </div>
                ...
                <div className="flex flex-col items-center justify-center py-20 text-center text-slate-500 bg-[#F8FAFC]/30 rounded-2xl border border-dashed border-slate-200">
                  ...
                  <p className="text-xs font-semibold">Aucun rapport radiologique ou fonctionnel n'a été enregistré.</p>
                </div>
              )}
            </div>
          </div>
