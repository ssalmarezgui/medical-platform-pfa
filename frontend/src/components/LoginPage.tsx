...
import { IconPill, IconLoader, IconLock, IconUser } from '@tabler/icons-react';
...
export const LoginPage = () => {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Enregistrement de la session dans le store Zustand
      loginUser(login, role, rawToken);
      setIsLoading(false);

      // --- REDIRECTION DYNAMIQUE SÉCURISÉE PAR RÔLE ---
      if (role === 'AGENT_IMMUNO') {
        navigate('/immuno-treatments');
      } else if (role === 'MEDECIN_INVESTIGATEUR') {
        navigate('/diagnostic-hub'); // Redirection de l'investigateur vers son espace de travail
      } else {
        navigate('/hospitalisation-hub'); // Fallback par défaut (ex: ADMIN)
      }

    } catch (err: any) {
      setIsLoading(false);
      console.error("Échec de connexion :", err.response?.data || err.message);
      setToastMessage(err.response?.data?.message || "Identifiant d'accès ou mot de passe incorrect.");
      setToastOpen(true);
    }
  };
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-3 rounded-xl border border-slate-200 outline-none font-bold text-slate-800 animate-none bg-white" 
                placeholder="" 
              />
            </div>
          </div>
