...
import { useHematologyHistory, useSaveHematology, useDeleteHematology } from '../features/hematology/hooks/useHematology';
import { usePatients } from '../features/patients/hooks/usePatients';
import { useQueryClient } from '@tanstack/react-query';
import { hematologyService } from '../features/hematology/api/hematologyService';
import { 
  IconDroplet, IconSearch, IconPlus, IconTrash, IconUserCheck, IconAlertCircle, IconCalendar, IconReportMedical, IconEdit,
...
export const HematologyPage = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[500px] text-center p-8 bg-white border border-red-100 rounded-[30px] shadow-sm max-w-lg mx-auto mt-12 text-xs">
        ...
        <p className="text-[#6588BB] text-sm leading-relaxed mb-6">
          La validation et la saisie des résultats biologiques d'hématologie et d'hémostase relèvent de la responsabilité légale exclusive du biologiste ou des agents de laboratoire.
        </p>
      </div>
    );
  }
  ...
  const { data: donorHematology, isLoading: loadingDonorHemato } = useDonorHematologyHistory(
    isDonorMode ? (selectedPatientId || undefined) : undefined
  );

  const hematology = isDonorMode ? donorHematology : patientHematology;
  const loadingHemato = isDonorMode ? loadingDonorHemato : loadingPatientHemato;

  const saveMutation = useSaveHematology();
  const deleteMutation = useDeleteHematology(patientIdString);

  const [localSearchTerm, setLocalSearchTerm] = useState('');

  const groupesSanguins = ["A Positif", "A Négatif", "B Positif", "B Négatif", "O Positif", "O Négatif", "AB Positif", "AB Négatif"];

  const listExamens = [
    "Globules rouges", "Hémoglobine/Hématocrite", "VGM", "CCMH/TCMH", "Globules Blancs",
    "Polynucléaires/Lymphocytes", "Monocytes/Eosinophiles", "Plaquettes", "Fer sérique/ Ferritinémie",
  ...
  useEffect(() => {
    if (hematology) {
      ...
      setPhenotypage(hematology.phenotypage || '');
      setAnalysesSaisies(hematology.analyses || []);
    } else {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isDonorMode) {
        await axios.post(`http://localhost:8081/api/hematologie/donneur/${selectedPatientId}`, payload);
      } else {
        await saveMutation.mutateAsync({ patientId: patientIdString, data: payload });
      }

      setToastType('success');
      setToastMessage("La fiche d'hématologie a été enregistrée avec succès !");
      setToastOpen(true);

      queryClient.invalidateQueries({ queryKey: ['hematologyHistory'] });
      queryClient.invalidateQueries({ queryKey: ['donorHematologyHistory'] });
    } catch {
      setToastType('error');
  ...
  const handleDelete = async () => {
    if (!hematology?.identifiantHH) return;
    try {
      if (isDonorMode) {
        await axios.delete(`http://localhost:8081/api/hematologie/${hematology.identifiantHH}`);
      } else {
        await deleteMutation.mutateAsync(hematology.identifiantHH);
      }
  ...
  const handleExportCSV = async (mode: 'global' | 'personal') => {
    ...
    if (mode === 'personal' && selectedPatientId) {
      ...
      filename = `export_hematologie_${isDonorMode ? 'donneur' : 'patient'}_${selectedPatientId}_${new Date().toISOString().split('T')[0]}.csv`;
    } else {
      try {
        const globalData = await hematologyService.getAll();
        datasetToExport = globalData || [];
        filename = `export_global_hematologie_cohorte_${new Date().toISOString().split('T')[0]}.csv`;
      } catch {
    ...
    if (datasetToExport.length === 0) {
      alert("Aucune donnée d'hématologie à exporter.");
      return;
    }
    ...
    if (mode === 'personal') {
      ...
      const csvRows = datasetToExport.map(row => [
        ...
        row.typeAnalyse || 'HEMATOLOGIE'
      ].join(";"));
      csvContent = "\ufeff" + [headers.join(";"), ...csvRows].join("\n");
    } else {
      const headers = ["patientId", "groupeSanguin", "phenotypage", "dateAna", "resultatAna", "valeurAna", "typeAnalyse"];
      const csvRows: string[] = [];
      datasetToExport.forEach(hh => {
        const patientId = hh.patientId || hh.patient?.identifiantP || "";
        const gp = hh.groupeSanguin || "";
        const ph = hh.phenotypage || "";
        
        if (hh.analyses && hh.analyses.length > 0) {
          hh.analyses.forEach((a: any) => {
            csvRows.push([
              patientId, gp, `"${ph.replace(/"/g, '""')}"`,
              a.dateAna || "", `"${a.resultatAna.replace(/"/g, '""')}"`, `"${a.valeurAna.replace(/"/g, '""')}"`,
              a.typeAnalyse || 'HEMATOLOGIE'
            ].join(";"));
          });
        } else {
          csvRows.push([patientId, gp, `"${ph.replace(/"/g, '""')}"`, "", "", "", "HEMATOLOGIE"].join(";"));
        }
      });
      csvContent = "\ufeff" + [headers.join(";"), ...csvRows].join("\n");
    }
  ...
  const handleDownloadTemplate = (mode: 'global' | 'personal') => {
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = mode === 'global' ? "modele_import_hematologie_global.csv" : "modele_import_hematologie_personnel.csv";
    link.click();
    URL.revokeObjectURL(url);
  };
    ...
    if (importMode === 'personal') {
      ...
      previewData.forEach(row => {
        nouvellesAnalyses.push({
          ...
          resultatAna: row.resultatAna || listExamens[0],
      ...
      previewData.forEach(row => {
        ...
        if (!mappedGroups[pid]) {
          mappedGroups[pid] = {
            groupeSanguin: row.groupeSanguin || groupesSanguins[0],
            phenotypage: row.phenotypage || '',
            analyses: []
          };
        }
      ...
      for (const [pid, data] of Object.entries(mappedGroups)) {
        try {
          await hematologyService.save(pid, data);
          successCount++;
        } catch (err) {
          console.error("Erreur import global :", err);
        }
      }
      setToastMessage(`${successCount} fiche(s) d'hématologie synchronisée(s) !`);
    }
      ...
      <div className="bg-white border border-[#A7C0E4]/30 rounded-[30px] p-8 shadow-sm mb-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-[#2B5296] flex items-center gap-2">
            <IconDroplet size={24} className="text-red-500 animate-pulse" />
        ...
        <div className="bg-white border border-[#A7C0E4]/30 rounded-[30px] p-8 shadow-sm">
          <div className="mb-6">
            <h3 className="text-base font-bold text-slate-800">
              {isDonorMode ? "Registre Hématologique de votre Cohorte Donneurs" : "Registre Hématologique de votre Cohorte Patients"}
            </h3>
            <p className="text-[#6588BB] text-xs mt-0.5 font-semibold">
              {isDonorMode 
                ? "Ouvrez le dossier d'un donneur pour dresser et sauvegarder sa carte d'analyses de sang."
                : "Ouvrez le dossier d'un patient pour dresser et sauvegarder sa carte d'analyses de sang."}
            </p>
          </div>
          ...
          <div className={`${isReadOnly ? 'lg:col-span-3' : 'lg:col-span-2'} bg-white border border-[#A7C0E4]/30 rounded-[30px] p-8 shadow-sm min-h-[500px] flex flex-col justify-between`}>
            <div>
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b">
                <div>
                  <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                    <IconDroplet size={22} className="text-red-500" />
                    Registre Hématologique ({isDonorMode ? `${selectedPatient?.prenomD} ${selectedPatient?.nomD}` : `${selectedPatient?.prenomP} ${selectedPatient?.nomP}`})
                  </h3>
                </div>
                ...
                <div className="flex gap-2 w-full sm:w-auto">
                  <div className="relative flex-1 sm:w-48">
                    ...
                    <input 
                      type="text" 
                      placeholder="Filtrer les analyses..." 
              ...
              <div className="grid grid-cols-2 gap-4 mb-6 bg-[#F8FAFC] p-4 rounded-2xl border border-slate-100/50">
                ...
                <div className="text-xs text-slate-600">
                  <p>Phénotypage : <strong className="text-slate-800">{phenotypage || 'Aucun'}</strong></p>
                </div>
              </div>
          ...
          <div className="bg-white rounded-[24px] p-8 w-full max-w-lg shadow-2xl space-y-6">
            <div className="flex justify-between items-center border-b pb-4">
              <h2 className="text-lg font-bold text-[#2B5296]">
                {importMode === 'global' ? "Importation de Cohorte Hématologique (CSV)" : `Importer les analyses de ${isDonorMode ? selectedPatient?.prenomD : selectedPatient?.prenomP}`}
              </h2>
