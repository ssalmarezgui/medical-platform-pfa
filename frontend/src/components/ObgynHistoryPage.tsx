...
export const ObgynHistoryPage = () => {
  ...
  // Remplissage automatique pour consultation
  useEffect(() => {
    if (obgynHistory) {
  ...
  return (
    <div className="max-w-[1200px] mx-auto py-8 text-xs">
      ...
      {/* --- ÉTAT A : VUE COHORTE (AUCUNE PATIENT SÉLECTIONNÉE) --- */}
      {!selectedPatientId ? (
        ...
        // --- ÉTAT B : VUE CLINIQUE PERSONNELLE (PATIENTE SÉLECTIONNÉE) ---
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          
          {/* Formulaire à gauche */}
          {(!obgynHistory || modeEdition) ? (
            <form onSubmit={handleSave} className="lg:col-span-1 bg-white border border-[#A7C0E4]/30 rounded-[30px] p-8 shadow-sm space-y-4 h-fit">
          ...
          <div className="lg:col-span-2 bg-white border border-[#A7C0E4]/30 rounded-[30px] p-8 shadow-sm min-h-[500px] flex flex-col justify-between">
            ...
            {/* Actions Crayon (Edit) et Corbeille (Delete) en bas à droite */}
            {obgynHistory && !modeEdition && (
