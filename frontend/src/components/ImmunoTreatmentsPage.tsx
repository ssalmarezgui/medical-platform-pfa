...
export const ImmunoTreatmentsPage = () => {
  ...
  const handleAddPrescription = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedTreatmentId || !selectedMedId || !datePremierePrise) return;
              ...
              {typeTIS === 'INDUCTION' ? (
                <div className="space-y-2 border-t pt-4 animate-in slide-in-from-top-2 duration-200">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <label className="flex items-center gap-1.5 cursor-pointer text-slate-700 font-semibold"><input type="checkbox" checked={atgTISI} onChange={(e) => setAtgTISI(e.target.checked)} /> ATG (Sérum)</label>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="mt-4 pt-3 border-t border-slate-100 flex justify-end gap-1.5" onClick={(e) => e.stopPropagation()}>
                          ...
                          <button onClick={() => triggerDelete(t.identifiantTIS!, 'TIS')} className="p-1.5 bg-red-50 text-red-500 hover:bg-red-100 rounded-lg border-none cursor-pointer"><IconTrash size={14} /></button>
                        </div>
                      </div>
                    );
                  })}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in slide-in-from-bottom-5 duration-300">
                ...
                <div className="bg-white border border-[#A7C0E4]/30 rounded-[30px] p-6 shadow-sm space-y-4">
                  ...
                  <form onSubmit={handleAddPrescription} className="space-y-3">
                    <div className="grid grid-cols-2 gap-2">
                      <select onChange={(e) => setSelectedMedId(Number(e.target.value))} className="w-full px-3 py-2 rounded-xl border border-slate-200 text-[11px] bg-white outline-none">
                        <option value="">Sélectionner Molécule...</option>
                        {listMeds?.map(m => <option key={m.identifiantMed} value={m.identifiantMed}>{m.nomCommercialMed} ({m.typeMed})</option>)}
                      </select>
                    ...
                    <table className="w-full text-left text-[11px]">
                      ...
                      <tbody className="divide-y">
                        {prescriptions && prescriptions.length > 0 ? (
                          ...
                          <tr><td colSpan={3} className="p-4 text-center text-slate-400">Aucune dialyse liée.</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
