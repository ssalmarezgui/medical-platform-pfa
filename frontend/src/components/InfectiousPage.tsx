...
import { useInfectiousHistory, useSaveInfectious, useDeleteInfectious } from '../features/infectious/hooks/useInfectious';
import { usePatients } from '../features/patients/hooks/usePatients';
import { useQueryClient } from '@tanstack/react-query';
import { infectiousService } from '../features/infectious/api/infectiousService';
import { 
  IconVirus, IconSearch, IconPlus, IconTrash, IconUserCheck, IconAlertCircle, IconCalendar, IconReportMedical, IconEdit,
...
import { useDonorInfectiousHistory } from '../features/infectious/hooks/useInfectious';
import { useAuthStore } from '../store/useAuthStore';

export const InfectiousPage = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[500px] text-center p-8 bg-white border border-red-100 rounded-[30px] shadow-sm max-w-lg mx-auto mt-12 text-xs">
        ...
        <p className="text-[#6588BB] text-sm leading-relaxed mb-6">
          La saisie, le dépistage et la validation des constantes de microbiologie et des sérologies virales (VIH, VHB, VHC, CMV...) relèvent de la responsabilité légale exclusive du biologiste ou des agents de laboratoire.
        </p>
      </div>
    );
  }
  ...
  const { data: patientInfectious, isLoading: loadingPatientInfectious } = useInfectiousHistory(
    !isDonorMode ? (patientIdString || undefined) : undefined
  );

  const { data: donorInfectious, isLoading: loadingDonorInfectious } = useDonorInfectiousHistory(
    isDonorMode ? (selectedPatientId || undefined) : undefined
  );

  const infectious = isDonorMode ? donorInfectious : patientInfectious;
  const loadingInfectious = isDonorMode ? loadingDonorInfectious : loadingPatientInfectious;

  const saveMutation = useSaveInfectious();
  const deleteMutation = useDeleteInfectious(patientIdString);

  const [localSearchTerm, setLocalSearchTerm] = useState('');

  const listExamensInfectieux = [
    "Uroculture",
    "TPHA, VDRL",
    "HIV",
    "Ag HbS",
  ...
  const [typeMS, setTypeMS] = useState("Enquête Infectieuse et Sérologie");
  
  const [selectedExamen, setSelectedExamen] = useState(listExamensInfectieux[0]);
  ...
  useEffect(() => {
    if (infectious) {
      setTypeMS(infectious.typeMS || "Enquête Infectieuse et Sérologie");
      setAnalysesSaisies(infectious.analyses || []);
    } else {
      setTypeMS("Enquête Infectieuse et Sérologie");
      setAnalysesSaisies([]);
    }
  }, [infectious]);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if (isDonorMode) {
        await axios.post(`http://localhost:8081/api/microbiologie/donneur/${selectedPatientId}`, payload);
      } else {
        await saveMutation.mutateAsync({ patientId: patientIdString, data: payload });
      }

      setToastType('success');
      setToastMessage("La fiche de microbiologie/sérologie a été enregistrée avec succès !");
      setToastOpen(true);

      queryClient.invalidateQueries({ queryKey: ['infectiousHistory'] });
      queryClient.invalidateQueries({ queryKey: ['donorInfectiousHistory'] });
    } catch {
      setToastType('error');
  ...
  const handleDelete = async () => {
    if (!infectious?.identifiantMS) return;
    try {
      if (isDonorMode) {
        await axios.delete(`http://localhost:8081/api/microbiologie/${infectious.identifiantMS}`);
      } else {
        await deleteMutation.mutateAsync(infectious.identifiantMS);
      }
      setConfirmOpen(false);
      setToastType('success');
      setToastMessage("Fiche de microbiologie/sérologie supprimée.");
      setToastOpen(true);
      ...
      queryClient.removeQueries({ queryKey: ['infectiousHistory'] });
      queryClient.removeQueries({ queryKey: ['donorInfectiousHistory'] });
    } catch {
      setConfirmOpen(false);
      setToastType('error');
  ...
  const handleExportCSV = async (mode: 'global' | 'personal') => {
    ...
    if (mode === 'personal' && selectedPatientId) {
      ...
      filename = `export_infectiologie_${isDonorMode ? 'donneur' : 'patient'}_${selectedPatientId}_${new Date().toISOString().split('T')[0]}.csv`;
    } else {
      try {
        const globalData = await infectiousService.getAll();
        datasetToExport = globalData || [];
        filename = `export_global_infectiologie_cohorte_${new Date().toISOString().split('T')[0]}.csv`;
      } catch {
    ...
    if (datasetToExport.length === 0) {
      alert("Aucune analyse d'infectiologie à exporter.");
      return;
    }
    ...
    if (mode === 'personal') {
      ...
      const csvRows = datasetToExport.map(row => [
        ...
        row.typeAnalyse || 'INFECTIEUX'
      ].join(";"));
      csvContent = "\ufeff" + [headers.join(";"), ...csvRows].join("\n");
    } else {
      const headers = ["patientId", "typeMS", "dateAna", "resultatAna", "valeurAna", "typeAnalyse"];
      const csvRows: string[] = [];
      datasetToExport.forEach(ms => {
        const patientId = ms.patientId || ms.patient?.identifiantP || "";
        const lib = ms.typeMS || "Enquête Infectieuse et Sérologie";
        
        if (ms.analyses && ms.analyses.length > 0) {
          ms.analyses.forEach((a: any) => {
            csvRows.push([
              patientId, `"${lib.replace(/"/g, '""')}"`,
              a.dateAna || "", `"${a.resultatAna.replace(/"/g, '""')}"`, `"${a.valeurAna.replace(/"/g, '""')}"`,
              a.typeAnalyse || 'INFECTIEUX'
            ].join(";"));
          });
        } else {
          csvRows.push([patientId, `"${lib.replace(/"/g, '""')}"`, "", "", "", "INFECTIEUX"].join(";"));
        }
      });
      csvContent = "\ufeff" + [headers.join(";"), ...csvRows].join("\n");
    }
  ...
  const handleDownloadTemplate = (mode: 'global' | 'personal') => {
    const headers = mode === 'global' 
      ? ["patientId", "typeMS", "dateAna", "resultatAna", "valeurAna"]
    ...
    const link = document.createElement("a");
    link.href = url;
    link.download = mode === 'global' ? "modele_import_infectious_global.csv" : "modele_import_infectious_personnel.csv";
    link.click();
    URL.revokeObjectURL(url);
  };
    ...
    if (importMode === 'personal') {
      ...
      previewData.forEach(row => {
        nouvellesAnalyses.push({
          ...
          resultatAna: row.resultatAna || "Uroculture",
      ...
      previewData.forEach(row => {
        ...
        if (!mappedGroups[pid]) {
          mappedGroups[pid] = {
            typeMS: row.typeMS || "Enquête Infectieuse et Sérologie",
            analyses: []
          };
        }
      ...
      for (const [pid, data] of Object.entries(mappedGroups)) {
        try {
          await infectiousService.save(pid, data);
          successCount++;
        } catch (err) {
          console.error("Erreur import global :", err);
        }
      }
      setToastMessage(`${successCount} fiches de cohorte importée(s) !`);
    }
  ...
  return (
    <div className="max-w-[1200px] mx-auto py-8 text-xs">
      ...
      <div className="bg-white border border-[#A7C0E4]/30 rounded-[30px] p-8 shadow-sm mb-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-bold text-[#2B5296] flex items-center gap-2">
            <IconVirus size={24} className="text-[#2B5296] animate-pulse" />
            Fiche de Microbiologie & Sérologie
          </h2>
        ...
        <div className="bg-white border border-[#A7C0E4]/30 rounded-[30px] p-8 shadow-sm">
          <div className="mb-6">
            <h3 className="text-base font-bold text-slate-800">
              {isDonorMode ? "Registre Infectiologique de votre Cohorte Donneurs" : "Registre Infectiologique de votre Cohorte Patients"}
            </h3>
            <p className="text-[#6588BB] text-xs mt-0.5 font-semibold font-sans">Consultez et complétez les données d'infectiologie et de microbiologie de vos candidats.</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-in fade-in duration-300">
            ...
            <div className="lg:col-span-1 bg-white border border-[#A7C0E4]/30 rounded-[30px] p-8 shadow-sm h-fit space-y-6">
              ...
              <form onSubmit={handleSave} className="space-y-4">
                <div>
                  ...
                  <input type="text" value={typeMS} onChange={(e) => setTypeMS(e.target.value)} required className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-xs font-bold text-slate-800 bg-white outline-none" />
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      ...
                      <select value={selectedExamen} onChange={(e) => setSelectedExamen(e.target.value)} className="w-full px-3 py-2 rounded-xl border border-slate-200 text-[10px] bg-white font-bold text-[#2B5296]">
                        {listExamensInfectieux.map(ex => <option key={ex} value={ex}>{ex}</option>)}
                      </select>
                    </div>
                    <div>
          ...
          <div className={`${isReadOnly ? 'lg:col-span-3' : 'lg:col-span-2'} bg-white border border-[#A7C0E4]/30 rounded-[30px] p-8 shadow-sm min-h-[500px] flex flex-col justify-between`}>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 pb-4 border-b">
                <div>
                  <h3 className="text-base font-bold text-slate-900 flex items-center gap-2">
                    <IconVirus size={22} className="text-[#2B5296]" />
                    Registre Infectiologique ({isDonorMode ? `${selectedPatient?.prenomD} ${selectedPatient?.nomD}` : `${selectedPatient?.prenomP} ${selectedPatient?.nomP}`})
                  </h3>
                </div>
                  ...
                  <table className="w-full border-collapse text-left text-xs">
                    ...
                    <tbody className="divide-y">
                      {filteredLocalAnalyses.length > 0 ? (
                        ...
                        <tr>
                          <td colSpan={isReadOnly ? 3 : 4} className="px-4 py-8 text-center text-slate-400 font-semibold">Aucun résultat d'analyse d'infectiologie saisi ne correspond à vos critères.</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
            ...
            {isReadOnly ? (
              ...
              <div className="text-[10px] text-slate-400 mt-6 border-t pt-4 italic font-sans">
                * Une fois les lignes ajoutées au tableau local, n'oubliez pas de cliquer sur "Sauvegarder la Fiche" en bas à gauche pour enregistrer définitivement le bilan d'infectiologie complet du patient.
              </div>
            )}
          </div>
          ...
          <div className="bg-white rounded-[24px] p-8 w-full max-w-lg shadow-2xl space-y-6">
            <div className="flex justify-between items-center border-b pb-4">
              <h2 className="text-lg font-bold text-[#2B5296]">
                {importMode === 'global' ? "Importation de Cohorte Infectiologique (CSV)" : `Importer les analyses de ${isDonorMode ? selectedPatient?.prenomD : selectedPatient?.prenomP}`}
              </h2>
      ...
      <DeleteConfirmModal isOpen={confirmOpen} onClose={() => setConfirmOpen(false)} onConfirm={handleDelete} title="Supprimer la fiche d'infectiologie ?" message="Cette action effacera définitivement l'intégralité du bilan de microbiologie et de sérologie ainsi que toutes les lignes de résultats associées." />
